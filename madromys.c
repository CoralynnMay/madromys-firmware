...
#endif
#ifndef DESKTOP_DRAG_DELTA
#    define DESKTOP_DRAG_DELTA 500
#endif
...
bool is_desktop_drag = false;
int16_t desktop_drag = 0;
...
report_mouse_t pointing_device_task_kb(report_mouse_t mouse_report) {
    ...
    } else if (is_desktop_drag) {
        int8_t deltaX = mouse_report.x;
        desktop_drag += deltaX;
        if (abs(desktop_drag) >= DESKTOP_DRAG_DELTA) {
            uint16_t drag_direction_key = desktop_drag < 0 ? KC_RIGHT : KC_LEFT;
            desktop_drag = 0;
            register_code(KC_LGUI);
            register_code(KC_LALT);
            tap_code(drag_direction_key);
...
bool process_record_kb(uint16_t keycode, keyrecord_t* record) {
    ...
    if (keycode == CM_DESKTOP_DRAG) {
        if (record->event.pressed) {
            is_desktop_drag = true;
        } else {
            is_desktop_drag = false;
            desktop_drag = 0;
        }
    }
